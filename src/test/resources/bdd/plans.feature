Feature: CRUD on Plans
  ...
  Scenario Outline: Update plan1
    Given A HTTP PUT request to <testcase> for id <id> with body <requestBodyPath> on <endPoint> returning status code <responseCode>
    Then Validate HTTP Response object <expectedOutputJSONPath>
    Examples:
      | testcase    | id | requestBodyPath                   | endPoint  | responseCode | expectedOutputJSONPath              |
      | update plan | 1  | testdata/request/plan1Update.json | /v1/plans | 200          | testdata/response/plan1Updated.json |
